...
########################################################################.
# BEREN 2022 - UP TO HERE - AFTER CLASS 25
########################################################################.
...
example(maxOuter=3, maxInner=6)



########################################################################.
# WILF 2022 - UP TO HERE - AFTER CLASS 24
########################################################################.
...
myt = function( m ){
 ...
 row = 1  # setup variables for the outer loop
 while ( row <= nrow(m) ) {    # condition for the outer loop
  
  # do something in the outer loop
  col = 1 # setup the variables for the inner loop
  while (col <= ncol(m) ){  # condition for the inner loop
   
   # do something in the inner loop
   #answer [ row , col  ]   = mat[ col , row  ]  # this works
   answer [ col , row  ]   = mat[ row , col  ]  # this works too (but only do one of them)
   
   # change a varaible so eventually the condition for the inner loop becomes false
   col = col + 1
  }
  
  # change something so that the condition for the outer loop eventually becomes FALSE
  row = row + 1
 }

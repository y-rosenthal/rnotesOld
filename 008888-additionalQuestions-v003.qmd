...
#
# The function should return a list that contains all of the "rotations" 
# of the vector. 
# Use a for loop or a while loop to write your function. (see examples)
#
# > rotateVector(c(10,20,30,40))
# [[1]]
# [1] 10 20 30 40
#
# [[2]]
...
#
# Write a function
#       quickPick = function( idnumber )
#
# The function should return 5 random integers. Each number should be 
# between 1 and 50. None of the numbers should appear twice.
# Whenever the same idnumber is provided, the same 5 random numbers should
# be returned (see examples).
#
# EXAMPLES
#   > quickPick(idnumber=100)
#   [1] 10 38 48 25 14
#
#   > quickPick(idnumber=200)
#   [1] 38 47 40 23 28
#
#   > quickPick(idnumber=100)  #(same results as first example)
#   [1] 10 38 48 25 14
#-----------------------------------------------------------------------------
...
#
# Write a function
#    numMatches = function(lotteryTicket, winningNumbers)
#
# lotteryTicket is expected to be a vector of 3 or more numbers (without duplicates).
# winningNumbers is expected to be a vector of numbers (without duplicates) the
# same length as lotteryTicket.
#
# If arguments are not in the expected format, the function should stop with an error.
# The function should return the number of values in lotteryTicket that match
# values in winningNumber (order doesn't matter) - see examples.
#
# EXAMPLES:
#   > numMatches(lotteryTicket=c(32,32),winningNumbers="apple")
#   Error in numMatches(lotteryTicket = c(32, 32), winningNumbers = "apple") : 
#     arguments are not in correct format
#
#   > numMatches(lotteryTicket=c(30,20,50),winningNumbers=c(1,50,30))
#   [1] 2
#
#   > numMatches(lotteryTicket=c(1,2,3,4),winningNumbers=c(10,20,30,40))
#   [1] 0
#-----------------------------------------------------------------------------

numMatches = function(lotteryTicket, winningNumbers){
  if (!is.numeric(lotteryTicket) | !is.numeric(winningNumbers) |
      length(unique(lotteryTicket)) != length(lotteryTicket) |
      length(unique(winningNumbers)) != length(winningNumbers) |
      length(lotteryTicket) < 3 | length(lotteryTicket) != length(winningNumbers)){
    stop("arguments are not in correct format")
  }
  
  sum(lotteryTicket %in% winningNumbers)
}

numMatches(lotteryTicket=c(32,32),winningNumbers="apple")
numMatches(lotteryTicket=c(32,20,50),winningNumbers=c(7,50,32))
numMatches(lotteryTicket=c(32,20,50,45),winningNumbers=c(7,9,42,1))
...
#
# Write a function
#         payout = function(lotteryTicket, winningNumbers, jackpot)
#
#   lotteryTicket is assumed to be a vector of 5 numbers
#   winningNumbers is assumed to be a vector of 5 numbers
#   jackpot is assumed to be a single positive number
#   (you do NOT have to check these assumptions)
#
# If all 5 numbers in the lottery ticket match the winning numbers (the
# order doesn't matter) then the payout is the entire jackpot.
#
# If 4 numbers in the lottery ticket match the winning numbers (the order
# doesn't matter) then the payout is 1/500'th of the jackpot.
#
# If 3 numbers in the lottery ticket match the winning numbers (the order
# doesn't matter) then the payout is 10.
#
# Otherwise the payout is zero.
#
# If the last number in the lotteryTicket is equal to
# the last number in the winningTicket then add 5 to the payout.
#
# EXAMPLES
#   > payout(c(10,20,30,40,50), c(30,40,50,1,2), 1000)
#   [1] 10
#
#   > payout(c(10,20,30,40,50), c(30,40,1,2,50), 1000)
#   [1] 15
#-----------------------------------------------------------------------------

payout = function( lotteryTicket, winningNumbers, jackpot){
  num = numMatches(lotteryTicket, winningNumbers)
  
  if(num == 5){
    payout = jackpot
  } else if (num == 4) {
    payout = 1/500 * jackpot
  } else if (num == 3) {
    payout = 10
  } else {
    payout = 0
  ...
  if (lotteryTicket[5] == winningNumbers[5]){
    payout = payout + 5
  }
  payout
}
...
payout(c(10,20,30,40,50), c(30,40,50,1,2), 1000)
payout(c(10,20,30,40,50), c(30,40,1,2,50), 1000)
payout(c(10,20,30,40,50), c(20,30,40,50,1), 1000)
payout(c(10,20,30,40,50), c(20,30,40,1,50), 1000)
payout(c(10,20,30,40,50), c(20,30,40,50,10), 1000)
payout(c(10,20,30,40,50), c(20,30,40,10,50), 1000)

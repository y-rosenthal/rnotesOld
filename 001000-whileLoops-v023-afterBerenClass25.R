...
fib = function( n ){
 ...
 if( !is.numeric(n) || trunc(n) != n || length(n) != 1 || n < 0){
  
  stop("n must be a single whole positive number")
 }
 ...
 # SETUP ANY VARIABLE THE LOOP NEEDS
 answer = c(0,1) 
 
 while( length(answer) != n ){        # CONDITION
  
  # DO SOMETHING
  lastNumber = answer[length(answer)]
  secondToLastNumber = answer[length(answer)-1]
  newNumber = lastNumber + secondToLastNumber
  
  answer = c( answer , newNumber    ) 
  
  # MAKE SURE THAT A VARIABLE IN THE CONDITION CHANGES SO THAT EVENTUALLY THE CONDITION BECOMES FALSE
 }
...
primesUpTo = function( n ){
 ...
 # SETUP VARIBALES BEFORFE THE LOOP THAT THE LOOP NEEDS
 
 numToCheck = 2
 while( numToCheck <= n  ){     # CONDITION

...
fib = function( n ){
 ...
 if (!is.numeric(n) || n < 1 || trunc(n) != n || length(n) != 1 ) {
 ...
 # SETUP ALL VARIABLES BY THE WHILE
 answer = c(0,1)
 
 while ( n > length(answer) ) {      # CONDITION
  
  # DO SOMETHING TO ACCOMPLISH YOUR GOAL
  answer = c( answer , answer[length(answer) - 1] + answer[length(answer)]  )
  
  # MAKE SURE THAT SOME VARIABLE CHANGES SO THAT THE CONDITION EVENTUALLY BECOMES FALSE
...
# same function inplemented with loops
primesUpTo = function( n ){

 
 # setup the variables for our loop
 answer = numeric(0)
 numToCheck = 2
 
 while( numToCheck <= n  ){  
  
    # do the code to accomplish your goal 
    if ( is.prime( numToCheck ) == TRUE ) {
     answer = c(answer, numToCheck)
    }
 
    # do SOMETHING in the loop to change the variables for the condition so it eventualy becomes false
...
firstNPrimes = function ( n ){
 ...
 # setup variables you need for the loop
 answer = numeric(0)
 numToCheck = 2
 
 while( length(answer) < n   ) {    # condition
  
  # do what needs to be done to accomplish your goal (for one iteration of the loop)
  if(is.prime(numToCheck) == TRUE) {
   answer = c(answer, numToCheck)
  }
  
  # make sure that some variable changes that will eventually cause the condition to be FALSE
  numToCheck = numToCheck + 1
 }
...
Fib = function(n ){
 ...
 while(n>2){
  q = r-2 #5-2 = 3
  w = r-1 #5-1 = 4
  sequence = c(sequence,sum(sequence[e-q],sequence[e-w])) #0,1,1+0, 
  e = e+1  # We need the sum of the new most recent 2 values, 
  n = n-1  # we need to get n more values - we got one - so subtract 1 from n

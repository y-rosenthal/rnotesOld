...
# The || symbol only works on the FIRST TRUE/FALSE on its left and right sides
FALSE || c(TRUE, FALSE, FALSE, TRUE )
...
divisors = function( n ){
 ...
 # make sure that n is a sinlge whole number
 if ( trunc(n) != n || !is.numeric(n) || length(n) != 1 ){
  stop ("n must be a single whole number")
 }
 
 theDivisors = numeric(0)

 # SETUP THE VARIABLE(S) THAT YOU NEED FOR THE LOOP
 numToCheck = 1 
 ...
 while( numToCheck <= n  ){     #CONDITION  
  ...
  # BODY OF THE LOOP
  if ( n %% numToCheck == 0) {
   theDivisors = c(theDivisors, numToCheck)
  }
...
mySum = function( nums ){
 ...
 # SETUP THE VARIBLES YOU NEED FOR THE WHILE LOOP
 
 theSum = 0
 place = 1
 
 while ( place <= length(nums) ) {    # CONDITION 

  # BODY OF LOOP 
  # KEEP ADDING UP NUMBERS
  theSum = nums[place] + theSum
...
countdown = function( from ) {
 ...
 while( from > 0 ){    # CONDITION
  ...
  # MAKE SURE THAT YOU CANGE SOMETHIGN TO EVENTUALLY MAKE THE CONDITION FALSE
  from = from - 1
  Sys.sleep(1)
 }
